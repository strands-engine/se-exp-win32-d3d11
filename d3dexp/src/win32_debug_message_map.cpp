// Based on Chili Direct3D Engine (Copyright 2018 PlanetChili <http://www.planetchili.net>) under GNU GPL
...
// added based on: https://microsoft.public.win32.programmer.ui.narkive.com/EVDpEGV5/undocumented-mysterious-window-message-0xae
...
namespace d3dexp::chili
{
	...
	[[nodiscard]] std::string win32_debug_message_map::operator() (DWORD msg_id, WPARAM wparam, LPARAM lparam) const noexcept
	{
		...
		if (iter != m_map.end())
		{
			oss << std::left << std::setw(first_column_width) << iter->second << std::right;
		}
		else
		{
